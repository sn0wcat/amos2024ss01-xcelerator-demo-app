...
export class XdMetricsService {
    ...
    public getMetricsForAsset(assetId: string, propertySetName: string) {
        return this.timeSeriesService.getTimeSeries({assetId, propertySetName, from: dayjs().subtract(60, 'minute').toDate()})
            .pipe(
                switchMap((data) => {
    ...
    private upsertMetrics(assetId: string, propertySetName: string, data: unknown): any {
        switch (propertySetName) {
            case 'PumpData': {
                ...
                return from(this.prismaService.metrics.deleteMany({
                    where: {
                        assetId: assetId,
                    }
                })).pipe(
                    ...
                    map((metrics) => metrics.map((m) => ({...pick(m, [ "name", "min", "max", "variance", "standardDeviation", "coefficientOfVariation", "mean" ])})))
                )
            }
        }
    }
